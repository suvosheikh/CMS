...
import { 
  ...
  PieChart as PieIcon, TrendingUp, Award, Tags, 
  Globe, Briefcase, Loader2, FileCheck, Table
} from 'lucide-react';
...
export const Reports: React.FC = () => {
  ...
  const handleExportCSV = () => {
    ...
    const headers = [
      "Entry ID", "Date", "Main Category", "Sub Category", "Brand/Type", 
      "Product Models", "Format", "Strategic Tag", "Campaign", "Status", "Asset Link", "Notes"
    ];

    const rows = filteredPosts.map(post => {
      const mainCat = categories.find(c => c.id === post.main_category_id)?.name || "N/A";
      const subCat = categories.find(c => c.id === post.sub_category_id)?.name || "N/A";
      const brand = post.brand_type_id ? (categories.find(c => c.id === post.brand_type_id)?.name || "N/A") : "N/A";
      ...
      return [
        ...
        post.status,
        `"${(post.asset_link || "").replace(/"/g, '""')}"`,
        `"${(post.notes || "").replace(/"/g, '""').replace(/\n/g, ' ')}"`
      ];
    });
    ...
    link.setAttribute("download", `RYANS-Workspace-Report-${filterMonth}.csv`);
  ...
  return (
    <div className="relative">
      <div id="report-content" className="space-y-12 pb-20 animate-in fade-in duration-700 p-2">
        <header className="flex items-center justify-between px-2 no-print">
          <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-8 py-3.5 bg-emerald-600 text-white rounded-2xl hover:bg-emerald-700 font-black text-xs uppercase tracking-widest transition-all shadow-xl shadow-emerald-100 active:scale-95"
            >
              <Download size={18} />
              Export CSV
            </button>
